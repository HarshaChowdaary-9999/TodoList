...
interface OnboardTodo{
    store:Instance<typeof TodoStore>
}
const OnboardTodo : React.FC<OnboardTodo>=observer(({store})=>{
    ...
    const OnboardTodo=store.todos.filter((todo)=>todo.progress=="InProgress")
    const closeb=()=>{
        store.onboardbutton=!store.onboardbutton
    }
    
    return(
        <div className={store.onboardbutton?"fixed  inset-0 flex-row items-center justify-center bg-black bg-opacity-50":"hidden"}>
